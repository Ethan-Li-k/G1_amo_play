...
class BaseBodyController:
    ...
    def __init__(self, robot_type, num_motors, joint_index_enum, joint_arm_index_enum, joint_lower_index_enum):
        ...
        # Override the hardcoded default targets with the current motor pose
        # before publish_thread starts. Otherwise the publish loop would
        # immediately drive every joint toward the hardcoded crouch pose
        # (kp=400 on waist, 300 on knee, 150 on hip) which yanks the robot
        # away from wherever the operator placed it. That yank is what
        # produced the violent first-tick swing visible in CSV
        # 20260509_103929 (waist_p went -0.12 → -0.01 in 24 ms with cmd
        # constant). Initializing q_target/lower_q_target to motorstate
        # makes the publish thread send "stay where you are" until an
        # external controller (maintain_standing / ctrl_whole_body) takes
        # over via ctrl_whole_body. tauff stays at the hardcoded values
        # because body_ik (pin.rnea) is not yet constructed at this point;
        # the ~1 Nm pose mismatch is absorbed by PD without visible motion.
        self.q_target = self.all_motor_q[15:].copy()
        self.lower_q_target = self.all_motor_q[:15].copy()
        logger.info(
            "G1_29_BodyController: q_target/lower_q_target seeded from current "
            "motorstate (no boot-time yank toward hardcoded default)."
        )
